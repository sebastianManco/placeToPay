...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2>{{ __('Editar Rol:') }} {{ $role->name }}</h2>
            <p class="text-muted mb-0">{{ __('Modifica la configuración del rol y los permisos granulares asociados.') }}</p>
        </div>
        ...
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-white fw-bold">
                ...
                @if ($role->isSystem() || in_array($role->slug, ['admin', 'client']))
                    <span class="badge bg-warning text-dark ms-2">{{ __('Rol Protegido del Sistema') }}</span>
                @endif
            </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6 mb-3">
                        <label for="slug" class="form-label fw-bold">
                            {{ __('Identificador (Slug)') }}
                            @if ($role->isSystem() || in_array($role->slug, ['admin', 'client']))
                                <small class="text-muted">({{ __('No modificable para roles de sistema') }})</small>
                            @else
                        ...
                        <input type="text"
                               id="slug"
                               name="slug"
                               class="form-control font-monospace @if($role->isSystem() || in_array($role->slug, ['admin', 'client'])) bg-light @endif @error('slug') is-invalid @enderror"
                               value="{{ old('slug', $role->slug) }}"
                               @if($role->isSystem() || in_array($role->slug, ['admin', 'client'])) readonly @else required @endif
                               maxlength="100">
                        @error('slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea id="description"
                                  ...
                                  placeholder="Describe las responsabilidades y alcance del rol...">{{ old('description', $role->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
