...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 fw-bold text-dark mb-1">{{ __('Historial de mis Pedidos') }}</h1>
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('orders.index') }}" class="row align-items-end">
                <div class="col-md-3 mb-2">
                    <label class="small fw-bold text-muted mb-1">{{ __('Referencia:') }}</label>
                    <input type="text" 
                           name="search" 
                                ...
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @elseif ($order->status === 'pending_payment')
                                            <span class="badge bg-warning px-2 py-1 text-dark">{{ __('Pendiente de Pago') }}</span>
                                    ...
                                    <td class="text-center align-middle">
                                        <div class="d-flex justify-content-center align-items-center">
                                            <a href="{{ route('orders.show', $order->id) }}" class="btn btn-outline-primary btn-sm me-1">
                                                {{ __('Ver Detalle') }}
                                            </a>
                                            @if ($order->canRetryPayment())
                                                <form action="{{ route('orders.retry-payment', $order->id) }}" method="POST" class="d-inline m-0">
                                                    ...
                                                    <button type="submit" class="btn btn-sm {{ $order->isRejected() ? 'btn-danger' : 'btn-warning' }}" title="{{ $order->isRejected() ? __('Reintentar Pago') : __('Pagar') }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" class="bi bi-credit-card me-1" viewBox="0 0 16 16">
